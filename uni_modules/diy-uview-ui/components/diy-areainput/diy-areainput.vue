<template>
	<view class="u-input" :class="{
		...
		<input class="u-input__input" type="text" :style="[getStyle]" :value="defaultValue" :placeholder="placeholder"
			...
			initDefaultValue(findValue) {
				if(findValue){
					...
					if(values.length>=1&&values[0]){
						let provinceIndex = provinces.findIndex((v, k) => {
					...
					if(values.length>=2&&values[1]){
						let cityIndex = changeCitys.findIndex((v, k) => {
							return v.code== values[1]
						});
						changeAreas = areas[cityIndex]
