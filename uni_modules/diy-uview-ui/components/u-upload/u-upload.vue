		...
		fileList: {
			...
			handler(val) {
				...
				val.map(value => {
					...
					if (!tmp) {
						let url = getApp().globalData.currentPage && getApp().globalData.currentPage.$tools? getApp().globalData.currentPage.$tools.renderImage(value.url):value.url
						lists.push({ url: url, error: false, progress: 100 });
					}
				});
